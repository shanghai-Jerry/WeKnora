...
func (t *HtmlInterpreterTool) Execute(ctx context.Context, args json.RawMessage) (*types.ToolResult, error) {
	...
	// Write the HTML report into the shared sandbox workspace so it can be
	// served alongside images generated by code_interpreter in the same directory.
	reportFileName := "report.html"
	reportPath := filepath.Join(t.workDir, reportFileName)
	logger.Infof(ctx, "[Tool][HtmlInterpreter] Writing report to workDir=%s, path=%s", t.workDir, reportPath)
	if err := os.WriteFile(reportPath, []byte(htmlContent), 0644); err != nil {
		logger.Errorf(ctx, "[Tool][HtmlInterpreter] Failed to write report file %s: %v", reportPath, err)
		reportFileName = ""
	} else {
		logger.Infof(ctx, "[Tool][HtmlInterpreter] Successfully wrote report file: %s", reportPath)
	}
	...
	if reportFileName != "" {
		resultData["file_path"] = reportFileName
		resultData["session_id"] = t.sessionID
	}
